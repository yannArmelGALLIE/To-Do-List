...
export const utilisateurInfo = async (req, res) => {
    ...
    try {
        ...
        if(!utilisateur) {
            res.status(400).json({ message: `Utilisateur inconnu : ${req.params.id}` });
        }
...
export const deleteUtilisateur = async (req, res) => {
    ...
    try {
        const utilisateur = await UtilisateurModels.findByIdAndDelete(req.params.id);
        if(!utilisateur) {
            res.send(400).json({ message: `Utilisateur introuvable: ${req.params.id}` });
        }
...
export const findAllTachesForUtilisateur = async (req, res) => {
    ...
    try {
        ...
        const tachesUtilisateur = await TacheModels.find({ utilisateur: req.params.id });
        res.status(201).json({tachesUtilisateur});
    }
    catch (err) {
        console.log(err);
        res.status(200).json({err});
